...
export default function SanityComments({ postId, comments }: SanityCommentsProps) {
  ...
  return (
    <div className="mt-20 pt-16 border-t border-slate-100 dark:border-slate-800">
      <h3 className="text-2xl font-black uppercase tracking-widest text-slate-900 dark:text-white mb-2">
        Comments &amp; Feedback
      </h3>
      ...
      <div className="space-y-8 mb-16">
        {comments?.length > 0 ? (
          comments.map((comment) => (
            <div key={comment._id} className="bg-slate-50 dark:bg-slate-900/40 p-6 md:p-8 rounded-[2rem] border border-slate-100 dark:border-slate-800">
              <div className="flex items-center gap-4 mb-4">
                <div className="w-12 h-12 bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 rounded-full flex items-center justify-center font-black text-lg">
                  {comment.name.charAt(0).toUpperCase()}
                </div>
                <div>
                  <h4 className="font-black text-slate-900 dark:text-white tracking-tight text-lg">{comment.name}</h4>
                  <p className="text-xs uppercase tracking-widest font-bold text-slate-400 dark:text-slate-500">
                    {formatDate(comment._createdAt)}
                  </p>
                </div>
              </div>
              <p className="text-slate-600 dark:text-slate-300 font-medium leading-relaxed pl-16">
                {comment.comment}
              </p>
            </div>
          ))
        ) : (
          <div className="p-8 text-center bg-slate-50 dark:bg-slate-900/40 border border-slate-100 dark:border-slate-800 rounded-[2rem]">
            <p className="text-slate-400 dark:text-slate-500 font-bold uppercase tracking-widest text-sm">No comments yet. Be the first!</p>
          </div>
        )}
      </div>

      {/* Comment Form */}
      <div className="bg-white dark:bg-slate-900 p-8 md:p-12 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-xl dark:shadow-none">
        <h4 className="text-xl font-black text-slate-900 dark:text-white mb-8 tracking-tight">Leave a Reply</h4>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div>
                ...
                <input
                  ...
                  className="w-full px-6 py-4 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl focus:outline-none focus:ring-2 focus:ring-red-500 focus:bg-white dark:focus:bg-slate-900 transition-all text-slate-900 dark:text-white font-medium"
                />
              </div>
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full px-6 py-4 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl focus:outline-none focus:ring-2 focus:ring-red-500 focus:bg-white dark:focus:bg-slate-900 transition-all text-slate-900 dark:text-white font-medium resize-none"
              ></textarea>
            </div>
